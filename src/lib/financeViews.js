import { workCompletedDate } from './workLedger.js';
...
export function statementPeriod(statement, cutoff, cases = []) {
  ...
  if (statement.kind === 'opening_balance') return statement.month < cutoff ? 'history' : 'opening';
  const dates = (statement.lineItems ?? []).map(l => l.date).filter(d => /^\d{4}-\d{2}-\d{2}$/.test(d ?? ''));
  for (const c of cases) {
    if (c.statementId === statement.id) {
      const date = workCompletedDate(c);
      if (date) dates.push(date);
    }
  }
  if (!dates.length) {
    // A monthly bill in the cutoff month cannot safely be assigned to a day.
    if (cutoff.slice(8) !== '01' && statement.month.slice(0, 7) === cutoff.slice(0, 7)) return 'mixed';
