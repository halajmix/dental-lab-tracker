...
import { X, Printer, FileText, Paperclip, MessageCircle, Loader2, Check, Download } from "lucide-react";
      ...
      */}
      ...
      <div ref={sheetRef} className="mx-auto my-6 max-w-[794px] bg-white p-10 text-[13px] leading-relaxed text-slate-800 shadow-2xl print-sheet print:my-0 print:max-w-none print:p-0 print:shadow-none">
        ...
        <div className="mt-5">
          ...
          {/* On screen: compact square-cropped preview grid. In PRINT these
              crops are useless to the lab, so they're hidden there and each
              photo instead gets its own full page below the signature. */}
        ...
        {/* signature + footer travel as ONE unbreakable unit with tighter
            print spacing — previously the generous screen margins pushed just
            these two blocks onto a nearly-blank page of their own before the
            photo appendix (seen in a real Safari print PDF). */}
        <div className="mt-10 print:mt-4 print:break-inside-avoid">
          <div className="text-[11px] text-slate-500">
            <p>Verified &amp; authorized by:</p>
            <p className="mt-6 border-t border-slate-400 pt-1 font-semibold text-slate-800">{caseObj.treatingDentistName || clinic.dentist}</p>
          </div>
          <p className="mt-6 border-t border-slate-200 pt-2 text-center text-[9px] text-slate-400 print:mt-3">
            Generated by Dr-Crown · This prescription is valid only with an authorized signature.
          </p>
        </div>

        {/* Print-only photo appendix — one photo per page, enlarged with its
            aspect ratio locked (natural proportions, contained within the
            page; never cropped or stretched). Mirrors the shared-PDF layout. */}
        {rx?.files
          ?.filter((f) => f.kind === "photo" && f.url)
          .map((f, i, all) => (
            <div key={`print-photo-${i}`} className="hidden break-before-page print:block">
