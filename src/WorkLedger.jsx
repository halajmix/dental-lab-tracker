...
import {fetchStatements,fetchStatementLineItems,fetchPayments,fetchManualLabWork,saveManualLabWork} from './lib/data.js';
import {workLedger,financeSummary,FINANCE_START,omanDate,workCompletedDate} from './lib/workLedger.js';
...
export function PaperWorkForm({lab,clinicsById,onSaved,onClose,existing}) {
  ...
  async function save(e){e.preventDefault();if(busy)return;setBusy(true);setError('');try{await saveManualLabWork(id,existing?.revision ?? null,entry);await onSaved?.();onClose();}catch(err){setError(err.message);}finally{setBusy(false);}}
  return <form onSubmit={save} className="space-y-4 rounded-2xl border border-blue-200 bg-white p-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      <label className="text-sm">Clinic<input required maxLength={200} list="paper-work-clinics" className={field} value={entry.clinicName} onChange={e=>update('clinicName',e.target.value)} /><datalist id="paper-work-clinics">{Object.values(clinicsById).map(c=><option key={c.id} value={c.name}/>)}</datalist></label>
    ...
    <div className="flex gap-2"><button disabled={busy} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{busy?'Saving…':'Save completed work'}</button><button type="button" disabled={busy} onClick={onClose} className="rounded-lg border px-4 py-2 text-sm">Cancel</button></div>
  </form>;
}
...
export default function WorkLedger({lab,clinicsById={},cases=[],summary=false,history=false,onReview}) {
  ...
  const accounts=useMemo(()=>summary?financeSummary(cases,data.statements,data.payments,clinicsById,lab.paperBalanceAsOf):[],[summary,cases,data,clinicsById,lab.paperBalanceAsOf]);
  ...
  return <section className="space-y-4">
    ...
    <p className="text-sm text-slate-600">{summary?'Completed work since 1 September 2026, payments applied since that date, and current outstanding bills including old debt. Work totals and outstanding bills overlap: do not add them together.':history?'Completed work before 1 September 2026. Payment status does not remove work.':'Work completed from 1 September 2026 onwards, grouped by clinic. Paid and unpaid work both stay here. Dates use Oman time.'}</p>
    ...
    {busy?<p className="text-sm text-slate-500">Loading…</p>:error?<div role="alert" className="text-red-700">{error}<button onClick={load} className="ml-3 underline">Retry</button></div>:<>
      ...
      <div className="flex gap-3"><input aria-label="Find work clinic" placeholder="Find a clinic…" className={field} value={query} onChange={e=>setQuery(e.target.value)}/>{summary&&<select aria-label="Payment status" className={field} value={status} onChange={e=>setStatus(e.target.value)}>{['all','Paid','Partially paid','Unpaid','Needs billing'].map(s=><option key={s} value={s}>{s==='all'?'All payment statuses':s}</option>)}</select>}</div>
      <div className="overflow-x-auto rounded-xl border bg-white"><table className="w-full text-sm"><thead className="bg-slate-50 text-left"><tr>{(summary?['Clinic','Completed work since 1 Sep','Payments applied since 1 Sep','Outstanding bills (all dates)','Completed, not billed','Unlinked receipts','Status','Review']:['Clinic','Work items','Total','Details']).map(h=><th key={h} className="whitespace-nowrap p-3">{h}</th>)}</tr></thead>
        <tbody>{visible.map(g=>summary?<tr key={g.key} className="border-t"><td className="p-3 font-semibold">{g.name}</td>{[g.work,g.paid,g.remaining,g.unbilled,g.unallocated].map((v,i)=><td key={i} className="whitespace-nowrap p-3">{money(v)}{i===3&&g.unpriced>0&&<span className="block text-xs text-amber-700">{g.unpriced} need pricing</span>}</td>)}<td className="p-3">{g.status}</td><td className="p-3"><button onClick={()=>onReview?.(g)} className="font-semibold text-blue-700">View bills</button></td></tr>:<React.Fragment key={g.key}>
      ...
      <p className="text-sm font-semibold">{summary?'Outstanding bills':'Completed work total'}: {money(visible.reduce((n,g)=>n+Math.round((summary?g.remaining:g.total)*1000),0)/1000)}</p>
      {summary&&<p className="text-xs text-slate-500">“Paid” means no recorded unpaid bill or completed work awaiting billing. Unlinked receipts are shown separately and are not deducted automatically. Clinics with unpaid bills and some recorded payment are marked Partially paid.</p>}
    </>}
  </section>;
}
